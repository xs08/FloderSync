...
- 文件事件只代表“可能变化”；必须通过 `git status --porcelain=v2` 判断是否真的需要同步。
- 文件变化触发在干净工作区应成功跳过，不创建空提交。
...
- 同类型策略最多一项：文件变化、固定间隔、每日指定时间。每日时间必须唯一且排序。
- 文件静默延迟范围 1 秒至 1 小时；固定间隔范围 1 分钟至 24 小时。
- 配置当前使用 schema v3。模型变化必须：
  1. 增加向后兼容解码或显式迁移；
  2. 保留 v1/v2 迁移测试；
  3. 原子写入并保留可恢复备份；
  4. 更新架构、测试方案和验证记录。
...
- `AutomationScheduler`、`FileChangeScheduler`、`CatchUpCalculator`：只生成同步意图，不执行 Git。
