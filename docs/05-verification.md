...
- 版本脚本只读检查与计算已通过：`0.1.0` 的下一 patch 版本为 `0.1.1`，`1.9.7` 为 `1.9.8`，非法两段格式会失败且不修改版本源。
- 本地化与 UI 定向 XCTest 通过；设置页中英文 960×640 离屏渲染已人工检查，浅色和深色下均直接显示 `0.1.0`，无截断或颜色依赖。
- 完整 XCTest 通过，普通测试构建后版本源仍为 `0.1.0`。
- 首次分发构建曾按已废弃的 minor 递增规则生成 `0.2.0`；该规则与产物已在确认正确语义后替换，不再作为有效发布版本。
- 修正后的默认分发构建从 `0.1.0` 递增到 `0.1.1`。通用 Release ZIP 同时包含 `arm64` 与 `x86_64`，原始 App 和解压后的 App 均通过 ad-hoc 签名校验，ZIP 完整性检查通过，SHA-256 为 `50947a32a02538f56843a37aea6d3f0bc228b328e306b4ce47dbe68d9c6d1656`；错误的 `0.2.0` ZIP 已移除，仓库 `build/` 已自动清理。
