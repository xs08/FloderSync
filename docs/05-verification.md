...
- 默认分发构建已将版本递增至 `0.2.0` 并生成通用 Release ZIP；产物同时包含 `arm64` 与 `x86_64`，原始 App 和解压后的 App 均通过 ad-hoc 签名校验，ZIP 完整性检查通过，SHA-256 为 `294c6964b2830de4f715d0c70de3bef1d078f81b460e0b3e3a6b503db6e348c3`。打包完成后仓库 `build/` 已自动清理；真实 Developer ID、公证及 `/Applications` 升级安装仍待发布阶段确认。
