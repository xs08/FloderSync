...
| 设置侧栏顶部留白过大 | 为标题栏控制区预留的固定 72pt 空白没有承载品牌信息 | 在不遮挡窗口控制按钮的居中区域展示 48pt 原色品牌图标，将顶部纯留白压缩为 30pt | 960×640 与最小 900×560 下均不得挤压导航或底部工具 |
| 安装后的 AppIcon 白底偏灰或随背景变化 | AppIcon 源文件背景透明，Finder 合成时透出所在表面颜色 | 将原有蓝青图形合成到不透明 `#FFFFFF` 母版并统一派生全尺寸资源 | 检查角点与透明区 alpha 为 1、RGB 为 255/255/255 |
...
- 一级侧栏宽 220pt，常驻且不可折叠。
- 侧栏顶部居中展示 48pt 原色 FloderSync 品牌图标，以图形填充标题栏控制区以下的空白，不重复显示产品文字。
